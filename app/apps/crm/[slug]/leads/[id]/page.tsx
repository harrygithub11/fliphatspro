...
import { analyzeSentiment, getSentimentColor, getSentimentEmoji } from '@/lib/sentiment';
...
export default function LeadProfilePage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        ...
        async function fetchStages() {
            try {
                const res = await fetch(`/api/admin/stages?t=${Date.now()}`);
        ...
        async function fetchScores() {
            try {
                const res = await fetch(`/api/admin/scores?t=${Date.now()}`);
                const data = await res.json();
    ...
    const currentScore = scores.find(s => s.value === lead?.score) || { color: 'red' };
    const currentStage = stages.find(s => s.value === lead?.stage) || { color: 'gray' };
    ...
    const handleCreateStage = async () => {
        ...
        try {
            ...
            if (res.ok) {
                const sRes = await fetch(`/api/admin/stages?t=${Date.now()}`);
                const sData = await sRes.json();
                if (sData.success) setStages(sData.stages);
                updateLead('stage', slug);
    ...
    const handleCreateScore = async () => {
        ...
        try {
            ...
            if (res.ok) {
                const sRes = await fetch(`/api/admin/scores?t=${Date.now()}`);
                const sData = await sRes.json();
                if (sData.success) setScores(sData.scores);
                updateLead('score', slug);
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (type === 'stage') {
                    ...
                    const sData = await sRes.json();
                    if (sData.success) setStages(sData.stages);
                    if (lead.stage === name) updateLead('stage', ''); // If deleted current
                } else {
                    const sRes = await fetch(`/api/admin/scores?t=${Date.now()}`);
                    const sData = await sRes.json();
                    if (sData.success) setScores(sData.scores);
                    if (lead.score === name) updateLead('score', '');
                }
            } else if (json.inUse) {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                await refreshData(); // Refresh profile in case it was migrated
                    ...
                    <CardHeader className="pb-4">
                        <div className="flex justify-between items-start">
                            ...
                            <Select
                                value={lead.score}
                                onValueChange={(val) => {
                                    if (val === 'add_new') setAddScoreOpen(true);
                                    else if (val === 'manage_scores') setManageScoresOpen(true);
                                    else updateLead('score', val);
                                }}
                            >
                                <SelectTrigger className={`w-[100px] h-8 text-xs uppercase tracking-wide border p-2 rounded-md ${COLOR_STYLES[currentScore.color] || COLOR_STYLES['red']}`}>
                                    <div className="flex items-center gap-1">
                                        {scores.find(s => s.value === lead.score)?.emoji || ''}
                                        <span className="truncate max-w-[80px]">
                                            {scores.find(s => s.value === lead.score)?.label || lead.score || 'Score'}
                                        </span>
                                    </div>
                                </SelectTrigger>
                                <SelectContent>
                        ...
                        <CardDescription className="flex items-center gap-2 mt-2">
                            <Select
                                value={lead.stage}
                                onValueChange={(val) => {
                                    if (val === 'add_new') setAddStageOpen(true);
                                    else if (val === 'manage_stages') setManageStagesOpen(true);
                                    else updateLead('stage', val);
                                }}
                            >
                                <SelectTrigger className="h-8 border text-sm p-2 bg-background hover:bg-muted/50 w-[160px] rounded-md">
                                    <div className="flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full inline-block ${currentStage.color ? `bg-${currentStage.color}-500` : 'bg-gray-500'}`}></div>
                                        <span className="truncate">
                                            {stages.find(s => s.value === lead.stage)?.label || lead.stage || 'Stage'}
                                        </span>
                                    </div>
                                </SelectTrigger>
                                <SelectContent>
                ...
                <Card className="h-full flex flex-col shadow-none border-l-0 rounded-l-none bg-transparent p-0">
                    <Tabs defaultValue="timeline" className="flex-1 flex flex-col h-full overflow-hidden">
                        <div className="shrink-0 pt-4">
                            <TabsList className="w-full flex border-b rounded-none p-0 h-auto bg-transparent gap-6 px-4">
                                <TabsTrigger
                                    value="timeline"
                                    className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-2 pb-3 pt-2 font-medium bg-transparent shadow-none transition-none"
                                >
                                    Timeline
                                </TabsTrigger>
                                <TabsTrigger
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                                ...
                                {filteredTimeline.map((item, i) => (
                                    <div key={item.id} className="flex gap-4 relative">
                                        ...
                                        <Avatar className={`w-8 h-8 shrink-0 z-10 ${!item.created_by_avatar ? (
                                            item.created_by_name === 'Admin User' ? 'bg-blue-500' :
                                                item.created_by_name === 'John Doe' ? 'bg-green-500' :
                                                    item.created_by_name === 'Sarah Smith' ? 'bg-purple-500' :
                                                        item.type === 'call_log' ? 'bg-blue-100' :
                                                            item.type === 'whatsapp_msg' ? 'bg-green-100' :
                                                                item.type === 'system_event' ? 'bg-zinc-100' : 'bg-purple-100'
                                        ) : ''
                                            }`}>
                                            {item.created_by_avatar && <AvatarImage src={item.created_by_avatar} alt={item.created_by_name || 'User'} className="object-cover" />}
                                            <AvatarFallback className={`text-xs font-bold ${item.created_by_name ? 'text-white bg-transparent' :
                                                item.type === 'call_log' ? 'text-blue-600 bg-transparent' :
                                                    item.type === 'whatsapp_msg' ? 'text-green-600 bg-transparent' :
                                                        item.type === 'system_event' ? 'text-zinc-600 bg-transparent' : 'text-purple-600 bg-transparent'
                                                }`}>
                                                {item.created_by_name ? (
                                                    item.created_by_name.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2)
                                                ) : (
                                                    <>
                                                        ...
                                                        {item.type === 'note' && <span className="text-[10px]">NT</span>}
                                                    </>
                                                )}
                                            </AvatarFallback>
                                        </Avatar>
                                        ...
                                        <div className="flex-1 pb-6">
                                            <div className="flex items-center justify-between mb-1">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {(item.type === 'call_log' || item.type === 'whatsapp_msg' || item.type === 'internal_note') && (() => {
                                                        const sentiment = analyzeSentiment(item.content);
                                                        return sentiment !== 'neutral' ? (
                                                            <Badge variant="outline" className={`text-[10px] uppercase font-bold px-1.5 h-5 ${getSentimentColor(sentiment)}`}>
