...
export default function NewYearOnlyPage() {
    // Configuration for this variant
    const PRICE = 5000;
    const ORIGINAL_PRICE = 15000;
    const SOURCE = 'newyear_5k';
    const VIDEO_SRC = '/Videos/5k LP.mp4'; // Original Video
                    ...
                    <div className="flex animate-marquee whitespace-nowrap gap-12 font-bold tracking-wider text-sm md:text-base items-center">
                        {[...Array(10)].map((_, i) => (
                            <span key={i} className="flex items-center gap-3">
                                <Zap className="h-4 w-4 fill-current" /> FOUNDER KICKSTARTER OFFER ₹ {PRICE} ONLY <Zap className="h-4 w-4 fill-current" />
                            </span>
                        ))}
                    </div>
                </div>
